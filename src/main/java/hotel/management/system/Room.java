        ...
        btnLoadData.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                try{
                    Conn c = new Conn();
                    if (c.stmt == null) {
                        JOptionPane.showMessageDialog(null, "Conn.stmt is NULL — connection failed silently.");
                        return;
                    }
                    ResultSet rs = c.stmt.executeQuery("select * from room");
                    int count = 0;
                    while(rs.next()) count++;
                    JOptionPane.showMessageDialog(null, "Rows fetched: " + count);

                    // re-run query since we consumed the ResultSet above
                    rs = c.stmt.executeQuery("select * from room");
